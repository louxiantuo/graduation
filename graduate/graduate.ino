...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(message[0] == 'C'){
    client.publish("outTopic","Machine2:reset");
    ESP.reset();
  }
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    client.publish("outTopic","WifiState3:true");
    key = digitalRead(16);
    if(key == 1)
    {
      client.publish("outTopic","MachineState3:true"); 
    }
    else
    {
      client.publish("outTopic","MachineState3:flase");
    }
    //ESP.reset();
  }
